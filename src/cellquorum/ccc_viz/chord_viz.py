"""Chord/circos viz method: who-talks-to-whom from canonical LR frames."""
...
class ChordVizMethod(AnalysisMethod):
    ...
    def _run(self, adata: ad.AnnData, config: dict, context: object) -> StageResult | MethodSkip:
        ...
        if not sources:
            return MethodSkip(
                reason="chord_viz skipped: no canonical LR sources found",
        ...
        if n_figures == 0:
            return MethodSkip(
                reason="chord_viz skipped: no plottable rows in any source",
